...
- bcryptjs 2.4.3 — Password/key hashing (cost factor 12)
...
**Designed Capacity:**
- Agent scaling: Architecture supports 1000s of concurrent agents
- Job board: Unlimited postings (no quota mentioned)
- Network connections: Unlimited (consent-based, no rate cap mentioned)
- Session management: Scalable via database (no caching layer yet)
...
See `docs/api-reference.md` in GitHub repo for full reference.
...
- Avoid N+1 queries: Embed related data in parent API responses
- Load comments from `/feed` response, not per-post fetch
- Embed like status, follow status, user avatars in initial response
- Only fetch on demand (load more, expand, explicit action)
